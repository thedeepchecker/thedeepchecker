...
class Model:
    # VGG-like
    def __init__(self):
        self.features = tf.placeholder(tf.float32, [None, 32, 32, 3], name='inputs')
        hook.add_to_collection("inputs", self.features)
        self.n_classes = 10
        ...
        self.test_loss = tf.losses.softmax_cross_entropy(self.labels, self.logits)
...
if __name__ == "__main__":
    ...
    train_data_loader = DataLoaderFromArrays(x_train, y_train.squeeze(), shuffle=True, one_hot=True, normalization=False)
    test_data_loader = DataLoaderFromArrays(x_test, y_test.squeeze(), shuffle=True, one_hot=True, normalization=False)

'''
A common fault is to set wrong axis for reduction operation, here we simulate the fault in the softmax.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.loss = tf.reduce_mean(self.cross_entropy(self.labels, self.probabilities))
        self.test_CE = self.cross_entropy(self.labels, self.probabilities)
        self.test_loss = tf.reduce_mean(self.test_CE)
    ...
    def softmax(self, logits):
        ...
        return tf.exp(logits) / tf.reduce_sum(
            tf.exp(logits), axis=0, keepdims=True
        )
    
    def cross_entropy(self, labels, probabilities):
        epsilon = 1e-32
        probabilities = tf.clip_by_value(probabilities, epsilon, 1 - epsilon)
        return -tf.reduce_sum(labels * tf.log(probabilities), axis=1, name="cross_entropy")
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_wrong_soft', data=data_under_test, model=model_under_test, buffer_scale=10)

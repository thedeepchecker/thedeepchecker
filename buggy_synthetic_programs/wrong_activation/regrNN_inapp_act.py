'''
We scale the output within the range of [-1,1], then, we set up sigmoid as last layer activation
which allows to the neural network yielding outputs between [0,1].
'''
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    data_loader_under_test = data.DataLoaderFromArrays(X_train, y_train, problem_type='regression', shuffle=True, one_hot=False, normalization=True, homogeneous=False, target_range=[-1.0,1.0])
    test_data_loader = data.DataLoaderFromArrays(X_test, y_test, problem_type='regression', shuffle=True, one_hot=False, normalization=True, homogeneous=False, target_range=[-1.0,1.0])
    model = Model()
    ...
    checker = DeepChecker(name='regrNN_inapp_act', data=data_under_test, model=model_under_test, buffer_scale=10)

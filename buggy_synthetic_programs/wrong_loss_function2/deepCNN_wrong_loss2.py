'''
A common fault is to set wrong axis for reduction operation, here we simulate the fault in the loss.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.CE = self.cross_entropy(self.labels, self.probabilities)
        self.loss = tf.reduce_mean(self.CE)
        self.test_CE = self.cross_entropy(self.labels, self.test_probabilities)
        self.test_loss = tf.reduce_mean(self.cross_entropy(self.labels, self.test_probabilities))
    ...
    def cross_entropy(self, labels, probabilities):
        ...
        return -tf.reduce_sum(labels * tf.log(probabilities), axis=0, name="cross_entropy")
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_wrong_loss_2', data=data_under_test, model=model_under_test, buffer_scale=10)

'''
Doing math operations (+, *, -,..) between tensors require an appropriate shaping to have the target 
broadcasting. Here, removing the explicit single dimension for the output [..., 1] for labels will make
the broadcasting in the loss calculation faulty.
'''
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='regrNN_wrong_loss', data=data_under_test, model=model_under_test, buffer_scale=10)
    checker.run_full_checks(overfit_batch=8, implemented_ops=[('diff',model.squared_diff)])
'''
To illustrate faulty DL program caused by high lr, we increase the lr in a way that hinders
the learning and the accuracy is dropped off.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        with tf.control_dependencies(update_ops):
            self.train_op = tf.train.MomentumOptimizer(learning_rate=0.1, momentum=0.9).minimize(self.loss)
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_high_lr', data=data_under_test, model=model_under_test, buffer_scale=10)

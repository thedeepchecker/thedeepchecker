'''
We deactivate the normalization in our data loader API object (DataLoaderFromArrays).
This would keep the data unscaled and mimic missing input normalization fault.  
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.labels, 1), tf.argmax(self.test_logits, 1)), tf.float32))
...
if __name__ == "__main__":
    ...
    data_loader_under_test = data.DataLoaderFromArrays(x_train, y_train.squeeze(), shuffle=True, one_hot=True, normalization=False)
    test_data_loader = data.DataLoaderFromArrays(x_test, y_test.squeeze(), shuffle=True, one_hot=True, normalization=False)
    model = Model()
    ...
    checker = DeepChecker(name='deep_CNN_miss_norm', data=data_under_test, model=model_under_test, buffer_scale=10)

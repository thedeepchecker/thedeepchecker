...
if __name__ == "__main__":
    ...
    data_loader_under_test = data.DataLoaderFromArrays(X_train, y_train, problem_type='regression', shuffle=True, one_hot=False, normalization=False, homogeneous=False)
    test_data_loader = data.DataLoaderFromArrays(X_test, y_test, problem_type='regression', shuffle=True, one_hot=False, normalization=False, homogeneous=False)
    model = Model()
    ...
    checker = DeepChecker(name='regrNN_miss_norm', data=data_under_test, model=model_under_test, buffer_scale=10)

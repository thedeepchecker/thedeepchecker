'''
We pass the softmax outputs(probabilities) to the softmax_cross_entropy_with_logits
that applies implicitly an inner softmax activation.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=self.probabilities))
        self.test_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=self.test_probabilities))
...
if __name__ == "__main__":
    ...
    checker = DNNChecker(name='deep_CNN_double_soft', data=data_under_test, model=model_under_test, buffer_scale=10)

'''
doing a lot of reduction over dimensions such as tf.reduce_sum(tf.reduce_mean(..., axis=...), axis=...)
and tf.reduce_mean(tf.reduce_sum(..., axis=...), axis=...) can lead to wrong use of one instead of the other.
In this example, we reproduce the inverse of mean and sum for loss computation. 
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.test_probabilities = self.softmax(self.test_logits)
        ...
        self.loss = tf.reduce_sum(self.cross_entropy(self.labels, self.probabilities))
        self.test_CE = self.cross_entropy(self.labels, self.test_probabilities)
        self.test_loss = tf.reduce_sum(self.test_CE)
    ...
    def cross_entropy(self, labels, probabilities):
        ...
        return -tf.reduce_mean(labels * tf.log(probabilities), axis=1, name="cross_entropy")
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_wrong_loss', data=data_under_test, model=model_under_test, buffer_scale=10)
    checker.run_full_checks(implemented_ops=[('softmax',model.test_probabilities), ('CE',model.test_CE)])
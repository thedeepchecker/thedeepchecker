'''
In case of implementing the gradient calculation from scratch, there is a risk of mistaken gradient direction
which inverses the problem to maximization. 
To introduce the resuting effect of this kind of fault in our ref code, we flipped sign of gradients.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        grads = tf.gradients((self.loss+self.reg_loss), tf.trainable_variables()) 
        neg_grads = [tf.negative(grad) for grad in grads]
        grads_and_vars = list(zip(neg_grads, tf.trainable_variables()))
        self.train_op = tf.train.GradientDescentOptimizer(0.1).apply_gradients(grads_and_vars)
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='base_CNN_flipped_sign', data=data_under_test, model=model_under_test, buffer_scale=10)

'''
We use a dummy initialization with arbitrary standard deviation that hinders the learning.
'''
import random
import math
import numpy as np
...
class Model:
    ...
    def __init__(self):
        ...
        self.train_op = tf.train.GradientDescentOptimizer(learning_rate=0.1).minimize(self.loss+self.reg_loss)#tf.train.AdamOptimizer(3e-4).minimize((self.loss+self.reg_loss))
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='base_CNN_random', data=data_under_test, model=model_under_test, buffer_scale=10)

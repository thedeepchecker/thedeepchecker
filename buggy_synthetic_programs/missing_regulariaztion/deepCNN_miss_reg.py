'''
We remove the dropout and batchnorm layers(equivalent to no regularized DNN) the lambda of l2-norm cost aiming at removing the regularization.  
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def build(self, features, n_classes, reuse=False, is_training=False):
        with tf.variable_scope('lenet', reuse=reuse):
            ...
            conv1_norm = conv1#tf.layers.batch_normalization(conv1, training=is_training)
            conv2 = tf.layers.conv2d(inputs=conv1_norm,
                                    ...
                                    use_bias=True,
                                    kernel_initializer=tf.initializers.he_uniform())
            conv2_norm = conv2#tf.layers.batch_normalization(conv2, training=is_training)
            ...
            conv3 = tf.layers.conv2d(inputs=pool2, 
                                    ...
                                    use_bias=True,
                                    kernel_initializer=tf.initializers.he_uniform())
            conv3_norm = conv3#tf.layers.batch_normalization(conv3, training=is_training)
            conv4 = tf.layers.conv2d(inputs=conv3_norm, 
                                    ...
                                    use_bias=True,
                                    kernel_initializer=tf.initializers.he_uniform())
            conv4_norm = conv4#tf.layers.batch_normalization(conv4, training=is_training)
            # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
            ...
            conv5 = tf.layers.conv2d(inputs=pool4, 
                                    ...
                                    use_bias=True,
                                    kernel_initializer=tf.initializers.he_uniform())
            conv5_norm = conv5#tf.layers.batch_normalization(conv5, training=is_training)
            conv6 = tf.layers.conv2d(inputs=conv5_norm, 
                                    ...
                                    use_bias=True,
                                    kernel_initializer=tf.initializers.he_uniform())
            conv6_norm = conv6#tf.layers.batch_normalization(conv6, training=is_training)
            # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
            ...
            reg_pool6_flat = tf.layers.flatten(pool6)
            # Fully connected layer 
            dense = tf.layers.dense(inputs=reg_pool6_flat, 
                                    units=128, 
                                    activation=tf.nn.relu,
                                    kernel_initializer=tf.initializers.he_uniform())
            dense_norm = dense#tf.layers.batch_normalization(dense, training=is_training)
            # Apply Dropout (if is_training is False, dropout is not applied)
            #reg_dense_norm = tf.layers.dropout(inputs=dense_norm, rate=0.5, training=is_training)
            outputs = tf.layers.dense(inputs=dense_norm,
                                      units=n_classes, 
                                      activation=None,
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_miss_dropout', data=data_under_test, model=model_under_test, buffer_scale=10)

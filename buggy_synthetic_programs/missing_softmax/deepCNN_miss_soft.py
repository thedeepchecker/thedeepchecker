'''
We remove the softmax layer from the neural network design to mimic forgetting it, which means the neural
network outputs logits instead of probabilities.
'''
import numpy as np
import pandas as pd
...
class Model:
    ...
    def __init__(self):
        ...
        self.probabilities = self.logits#tf.nn.softmax(self.logits)
        self.test_logits = self.build(self.features, self.n_classes, reuse=True, is_training=False)
        self.test_probabilities = self.test_logits#tf.nn.softmax(self.test_logits)
        ...
        self.loss = self.cross_entropy(self.labels, self.probabilities)
        self.test_loss = self.cross_entropy(self.labels, self.test_probabilities)
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='deep_CNN_miss_soft', data=data_under_test, model=model_under_test, buffer_scale=10)

...
class Model:
    ...
    def __init__(self):
        ...
        self.probabilities = self.logits#tf.nn.softmax()
        self.acc = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.labels, 1), tf.argmax(self.logits, 1)), tf.float32))
        self.reg_loss = tf.losses.get_regularization_loss()
        self.loss = self.cross_entropy(self.labels, self.probabilities)#tf.losses.softmax_cross_entropy(self.labels, self.logits)
...
if __name__ == "__main__":
    ...
    checker = DeepChecker(name='baseCNN_miss_soft', data=data_under_test, model=model_under_test, buffer_scale=10)
